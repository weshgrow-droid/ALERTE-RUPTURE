...
const APP_VERSION = 'v2026-09-10-12h30-recalibrate';
...
function rebuildVarieties(){
  ...
  for (const s of STATE.sales){
    ...
    if (norm.ignore) continue; // "Ignore import = OUI" dans la table de correspondance -> on respecte ce signal explicite
...
function computeWeeklyForecast(v, weekStart, refWeekStart){
  // Tendance : moyenne PONDÉRÉE des TENDANCE_WEEKS dernières semaines
  // complètes — la semaine la plus récente pèse le plus (poids 8), la plus
  // ancienne le moins (poids 1), pour que la tendance réagisse plus vite à
  // un changement récent plutôt que d'être diluée à parts égales sur 8 semaines.
  let weightedSum = 0, weightTotal = 0;
  for (let w=1; w<=TENDANCE_WEEKS; w++){
    const weight = TENDANCE_WEEKS - w + 1; // w=1 (semaine dernière) -> poids le plus fort
    weightedSum += (v.weeklyTotals.get(refWeekStart - 7*w) || 0) * weight;
    weightTotal += weight;
  }
  const tendance = weightedSum / weightTotal;
...
function buildForecastModel(v, refSerial){
  ...
  // Recalage intra-semaine : si les jours déjà réels de la semaine EN COURS
  // vendent plus (ou moins) que ce que le modèle prévoyait pour ces mêmes
  // jours, on applique le même écart aux jours restants de cette semaine —
  // pour que la prévision réagisse aux ventes déjà connues cette semaine,
  // pas seulement à la moyenne historique.
  ...
  for (let d=0; d<7; d++){
    ...
    if (date < refSerial && v.ventesParDate.has(date)){
      ...
      realizedExpected += weeklyForecasts[0].forecast * weights[d];
    }
  }
  ...
  return { weights, weeklyForecasts, refWeekStart, currentWeekRatio };
}
...
function forecastSaleForDate(model, dateSerial){
  ...
  let forecast = wf.forecast * model.weights[dow];
  // Recalage intra-semaine : seulement pour les jours restants de la semaine
  // en cours (weekIdx===0), sur la base de ce qui a déjà été vendu cette
  // même semaine.
  if (weekIdx === 0) forecast *= model.currentWeekRatio;
  return forecast;
}
...
function diagnoseSale(s){
  ...
  if (norm.ignore) return { statut: 'Exclu (Ignore import = OUI)', qtyFinale: null, variete: null };
