...
const APP_VERSION = 'v2026-09-10-16h20-stockcount';
...
function diagnoseSale(s){
  ...
  const inScope = norm.key && masterKeysSet.has(norm.key);
  ...
  const v = STATE.varieties.get(norm.key);
  let qty = s.qty;
  const colis = isColisEntry(s.codeRaw, s.nomRaw) || isColisEntry(s.codeRaw, norm.nom);
  if (colis) qty = qty * colisMultiplier(v.section);
  if (v.section === 'MICROXS') qty = qty / 2;
  const detail = colis ? `colis ×${colisMultiplier(v.section)}${v.section==='MICROXS'?' puis ÷2 (XS)':''}` : (v.section==='MICROXS' ? 'unité ÷2 (XS)' : 'unité');
...
function diagnoseProd(p){
  ...
  if (!inScope) return { statut: 'Hors périmètre / non reconnu', qtyFinale: null, variete: null };
  const v = STATE.varieties.get(norm.key);
  return { statut: p.inWindow ? 'Compté' : 'Hors fenêtre ±45j (ignoré)', qtyFinale: p.qty, variete: v.nom };
}
