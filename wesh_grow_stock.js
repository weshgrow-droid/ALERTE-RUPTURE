...
const APP_VERSION = 'v2026-09-02-21h55-datewindow';
...
function parseProductionRows(rows2D, refSerial){
  ...
  const out = [];
  let rejectedOutOfWindow = 0, rejectedBadDate = 0;
  for (let b=0; b<headerIdxs.length; b++){
    ...
    for (let i=headerIdx+1; i<blockEnd; i++){
      ...
      // On ne garde que les dates dans une fenêtre raisonnable autour
      // d'aujourd'hui : ce fichier contient aussi tout un historique de
      // production passée (même format d'en-tête, mois/jours sans année),
      // et notre déduction d'année par proximité n'est fiable que pour des
      // dates proches de la référence — au-delà, on préfère écarter la ligne
      // plutôt que d'afficher une date fausse.
      if (eth < refSerial - 3 || eth > refSerial + 45){ rejectedOutOfWindow++; continue; }
      out.push({ date: eth, codeRaw: produit, qty });
    }
  }
  return { rows: out, headerFound: true, blocksFound: headerIdxs.length, rejectedOutOfWindow, rejectedBadDate };
}


// ---------- État global ----------
const TARGET_BUSINESS_DAYS = 20; // 4 semaines ouvrées (lundi->vendredi), affichées à la suite
...
// période affichée (TARGET_BUSINESS_DAYS jours ouvrés, à partir du lundi de la semaine en
...
function projectVarietyFull(key){
  ...
  let d = weekStart;
  let guard = 0;
  // On avance jour civil par jour civil (le week-end fait aussi évoluer le
  // stock si jamais il y a des données ce jour-là, pour ne rien perdre),
  // mais on n'affiche que les jours ouvrés (lundi->vendredi) — le stock du
  // lundi matin reprend donc naturellement là où le vendredi s'est arrêté.
  while (days.length < TARGET_BUSINESS_DAYS && guard < 400){
    ...
    if (isWeekday(d)){
      days.push({ date: d, stock: running, production: prod, vente, venteReelle, isMonday: serialToDate(d).getUTCDay()===1 });
    }
    d++;
    guard++;
  }
...
async function refreshAll(){
  ...
  try{
    ...
    if (!parsedProd.headerFound){
      ...
      logImport(`${STATE.production.length} lots de production retenus (fenêtre ±45j autour d'aujourd'hui) dans l'onglet « ${parsedProd.sheetTitle} » (${parsedProd.blocksFound} tableau(x) fusionné(s), ${parsedProd.rejectedOutOfWindow} lignes hors fenêtre écartées)${parsedProd.scanned ? ' — détecté automatiquement' : ''}.`);
    }
...
function renderKPIs(){
  ...
  const kpis = [
    ...
    { label: `Ruptures anticipées (${TARGET_BUSINESS_DAYS}j ouvrés)`, value: ruptureCount, danger: true },
...
function renderDispoContinuous(){
  ...
  const dayHeaders = [];
  // On génère les mêmes 20 jours ouvrés en regardant une variété de
  // référence (n'importe laquelle, la séquence de dates est identique pour
  // toutes) pour construire l'en-tête une seule fois.
  ...
  for (const day of refDays){
    ...
    const weekBorder = day.isMonday ? ' style="border-left:3px solid var(--primary);"' : '';
    dayHeaders.push(`<th class="${isToday?'today':''}"${weekBorder}><div class="day-head ${isToday?'today':''}"><span class="dow">${dowFr(day.date)}</span><span class="dm">${fmtDateShort(day.date)}</span></div></th>`);
  }

  let bodyRows = '';
  let totalVarieties = 0;
  const colCount = 1 + refDays.length;
  for (const sec of allSections()){
    const keys = sectionKeys(sec);
    if (keys.length === 0) continue;
    ...
    for (const key of keys){
      ...
      for (const day of proj.days){
        ...
        const weekBorder = day.isMonday ? ' style="border-left:3px solid var(--border);"' : '';
        cells += `<td${weekBorder}>
...
const TAB_DESCRIPTIONS = {
  dispo: `Stock projeté jour par jour (${TARGET_BUSINESS_DAYS} jours ouvrés à la suite, week-ends masqués) : stock de départ + production entrante − ventes (réelles si connues, sinon prévision).`,
