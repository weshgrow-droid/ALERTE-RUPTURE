...
// renommé). On essaie d'abord le dernier onglet qui a fonctionné, puis on
// scanne tous les onglets du fichier si besoin.
...
async function findAndLoadProductionTable(spreadsheetId, refSerial){
  if (cachedProductionSheetTitle){
    try{
      const rows = await sheetsGetValues(spreadsheetId, `'${cachedProductionSheetTitle}'!A1:CZ3000`);
      const parsed = parseProductionRows(rows, refSerial);
      if (parsed.headerFound) return { ...parsed, sheetTitle: cachedProductionSheetTitle, scanned: false };
...
function buildProductMap(rows){
  ...
  const headerInfo = `Code Produit=${iCodeProduit>=0?'ok':'MANQUANT'}, Code Maj=${iCodeMaj>=0?'ok':'MANQUANT'}, Secteur MAJ=${iSecteurMaj>=0?'ok':'MANQUANT'}, Nom produit=${iNomProduit>=0?'ok':'MANQUANT'}`;
