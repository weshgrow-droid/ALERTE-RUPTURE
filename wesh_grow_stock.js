...
const APP_VERSION = 'v2026-09-10-14h30-demandcorrection';
...
function buildForecastModel(v, refSerial){
  ...
  if (realizedExpected > 0){
    ...
    currentWeekRatio = Math.max(MOMENTUM_MIN, Math.min(MOMENTUM_MAX, currentWeekRatio));
  }
    ...
    for (const key of keys){
      ...
      for (const day of proj.days){
        ...
        if (day.isFriday){
          ...
          cells += `<td class="${prevClass}" style="background:#FAF7EE;border-left:2px solid var(--accent);">
            ...
            <div style="margin-top:4px;">
              <span style="font-size:9px;color:var(--muted);">Demande manquée</span>
              <input type="number" class="demand-correction-input" data-key="${key}" data-week="${weekKeyForCorrection}" value="${existingCorrection||''}" placeholder="0" title="Ventes perdues à cause d'une rupture cette semaine — réparties sur les jours en rupture pour corriger les prévisions" style="width:100%;text-align:center;font-size:10px;padding:1px;">
            </div>
          </td>`;
