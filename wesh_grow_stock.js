...
const APP_VERSION = 'v2026-09-10-15h15-entertrigger';
...
function buildForecastModel(v, refSerial){
  ...
  // Tendance historique : moyenne simple des TENDANCE_WEEKS dernières
  // semaines complètes (base de référence, avant recalage sur "maintenant").
  let sum = 0;
  ...
  return { weights, weeklyForecasts, refWeekStart, tendance, currentWeekRatio, adjustedBase };
}
...
async function savePersistedState(silent){
  try{
    await sheetsUpdateValues(APP_STATE_CONFIG.spreadsheetId, `'${APP_STATE_CONFIG.sheetTitle}'!A2:B2`,
      [['appState', JSON.stringify({ ethOffsets: STATE.ethOffsets, stockOverrides: STATE.stockOverrides, weeklyDemandCorrections: STATE.weeklyDemandCorrections })]]);
...
function renderDispoContinuous(){
  ...
  for (const sec of allSections()){
    ...
    for (const key of keys){
      ...
      const m = proj.model;
      const debugLine = `<div class="cell-detail" style="font-size:9px;color:var(--muted);line-height:1.5;" title="Tendance historique × ratio de la semaine en cours = base réactualisée, puis × momentum (an dernier) par semaine future">
        Tend. ${m.tendance.toFixed(0)} · ratio ${m.currentWeekRatio.toFixed(2)} · base ${m.adjustedBase.toFixed(0)}<br>
        Mom. S+1: ${m.weeklyForecasts[1] ? m.weeklyForecasts[1].momentum.toFixed(2) : '–'} (conf. ${m.weeklyForecasts[1] ? m.weeklyForecasts[1].confidence : '–'})
      </div>`;
  ...
  return `
    <button type="button" id="togglePrevWeekBtn" class="prev-week-toggle" style="margin-bottom:8px;">👁 Afficher la semaine précédente</button>
    <div class="grid-wrap">
      <table class="grid">
        <thead>
          <tr>
