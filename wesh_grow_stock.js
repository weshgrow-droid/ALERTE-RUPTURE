...
const APP_VERSION = 'v2026-09-10-15h45-adaptivetendance';
...
function buildForecastModel(v, refSerial){
  ...
  const adjustedBase = tendance * currentWeekRatio + (v.currentWeekDemandCorrection || 0);
  ...
  // par construction, donc sa prévision = adjustedBase directement).
  ...
  for (let w=0; w<weeksNeeded; w++){
    ...
    weeklyForecasts.push({ tendance, momentum, confidence, varietyMomentum, marketMomentum, forecast: adjustedBase * momentum });
  }

  return { weights, weeklyForecasts, refWeekStart, tendance, tendanceCourte, tendanceLongue, tendanceWeeksLong, memeSens, currentWeekRatio, adjustedBase };
}
