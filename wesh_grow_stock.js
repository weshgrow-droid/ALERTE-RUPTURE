...
// On repère les colonnes par leur intitulé (pas par position fixe), pour ne
// pas dépendre de l'ordre exact des colonnes dans "Product agregate".
function buildProductMap(rows){
  const map = new Map();
  if (!rows || rows.length < 2) return { map, headerInfo: 'vide ou introuvable' };
  const header = rows[0];
  const findCol = (name) => {
  ...
  let iCodeMaj = findCol('Code Maj');
  let iSecteurMaj = findCol('Secteur MAJ');
  const iIgnoreImport = findCol('Ignore import');
  // Repli sur la position habituelle (D et E) si l'intitulé exact ne matche pas
  // (accent, casse, espace différents dans le Sheet réel).
  let codeMajBy = 'nom', secteurMajBy = 'nom';
  if (iCodeMaj === -1){ iCodeMaj = 3; codeMajBy = 'position D'; }
  if (iSecteurMaj === -1){ iSecteurMaj = 4; secteurMajBy = 'position E'; }
  ...
  const headerInfo = `Code Produit=${iCodeProduit>=0?'ok':'MANQUANT'}, Code Maj=${codeMajBy}, Secteur MAJ=${secteurMajBy}, Nom produit=${iNomProduit>=0?'ok':'MANQUANT'} — en-têtes vus: [${header.map(h=>String(h||'').trim()).join(' | ')}]`;
  return { map, headerInfo };
}
...
function rebuildVarieties(){
  ...
  const candidates = new Map(); // codeMaj -> { nom, section }
  for (const rawCode of STATE.sortiesCodes){
    const { key, nom, unmapped, secteurMaj } = normalizeToBase(rawCode);
    if (!key) continue;
    const baseCode = stripProductionSuffix(String(rawCode).trim().toUpperCase());
    let section = secteurMaj ? String(secteurMaj).trim().toUpperCase() : null;
    if (!section) section = guessSectionFromCode(baseCode);
    if (section === 'FLEURS') continue; // exclusion volontaire des fleurs pour l'instant
    if (unmapped) STATE.unmappedCodes.add(rawCode.trim().toUpperCase());
    if (!candidates.has(key)) candidates.set(key, { nom: nom || key, section });
  }

  for (const [key, info] of candidates.entries()){
    STATE.varieties.set(key, { nom: info.nom, section: info.section, ventesParDate: new Map(), productionParDate: new Map() });
  }
  const masterKeys = new Set(STATE.varieties.keys());

  for (const s of STATE.sales){
    const { key } = normalizeToBase(s.codeRaw);
    if (!key || !masterKeys.has(key)) continue; // hors périmètre SortiesRecoltes -> ignoré
...
async function refreshAll(){
  ...
  try{
    ...
    const built = buildProductMap(prodAggRows);
    STATE.productMap = built.map;
    logImport(`${STATE.productMap.size} correspondances produit chargées (${built.headerInfo}).`);
    ...
    logImport('Chargement du périmètre des variétés (SortiesRecoltes)…');
    const sorties = await findSortiesRecoltesCodes(PRODUCTION_SHEET_CONFIG.spreadsheetId);
    STATE.sortiesCodes = sorties.codes;
    STATE.sortiesSheetTitle = sorties.sheetTitle;
    if (!sorties.sheetTitle){
      logImport(`⚠️ Aucun onglet SortiesRecoltes n'a été trouvé (onglets vérifiés : ${(sorties.scannedTitles||[]).join(', ')}). Aucune variété ne peut être affichée sans lui.`);
    } else {
      logImport(`${STATE.sortiesCodes.size} codes distincts trouvés dans « ${sorties.sheetTitle} »${sorties.scanned ? ' (détecté automatiquement)' : ''} — c'est le périmètre suivi.`);
    }
    ...
    if (!parsedProd.headerFound){
      ...
      logImport(`${STATE.production.length} lots de production trouvés dans l'onglet « ${parsedProd.sheetTitle} »${parsedProd.scanned ? ' (détecté automatiquement)' : ''}.`);
    }
    ...
    document.getElementById('headerMeta').innerHTML =
      ...
      `<span class="small">${STATE.varieties.size} variétés suivies (périmètre : ${STATE.sortiesSheetTitle || 'introuvable'}) · ` +
