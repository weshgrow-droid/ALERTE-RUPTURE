...
const APP_VERSION = 'v2026-09-11-17h30-chart';
...
function renderVarietyChartSVG(key){
  ...
  let crossing = null;
  for (let i=todayIdx; i<weeksRange.length-1; i++){
  ...
  for (let i=0; i<weeksRange.length-1; i++){
  ...
  svg += drawLine(sales, '#2a78d6');
  svg += drawLine(prod, '#eb6834');
  ...
  const alertHtml = crossing
    ...
    : `<div style="margin-top:8px;padding:8px 12px;border-radius:6px;background:var(--ok-bg);color:var(--primary);font-size:12px;">Pas de croisement détecté — couverture stable.</div>`;
