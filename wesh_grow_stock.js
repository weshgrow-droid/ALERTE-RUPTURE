...
const APP_VERSION = 'v2026-09-02-23h45-momentum';
...
function rebuildVarieties(){
  ...
  for (const section of ['MICROPOUSSES','MICROXS','JP']){
    for (const code of PARIS_18_SCOPE[section]){
      ...
      STATE.varieties.set(key, {
        ...
        weeklyTotals: new Map(),   // weekStart -> qty (pour Tendance & Momentum)
        weeklyClients: new Map()  // weekStart -> Set(client) (pour l'indicateur de confiance)
      });
  ...
  for (const s of STATE.sales){
    ...
    // Totaux hebdo + clients distincts par semaine, nécessaires au calcul du
    // momentum et de l'indicateur de confiance (façon "Prev Semis").
    const wk = mondayOfWeek(s.date);
    v.weeklyTotals.set(wk, (v.weeklyTotals.get(wk)||0) + qty);
    if (!v.weeklyClients.has(wk)) v.weeklyClients.set(wk, new Set());
    v.weeklyClients.get(wk).add(String(s.client||'').trim().toLowerCase());
  }
  // Momentum "marché" : volume hebdo pondéré sur TOUTES les variétés Paris 18
  // combinées, XS divisées par 2 (2 barquettes XS = 1 équivalent-S, le
  // raisonnement de semis se fait en base S) — sert de repli fiable quand une
  // variété seule n'a pas assez de clients distincts pour être un signal sûr.
  STATE.marketWeekly = new Map();
  for (const v of STATE.varieties.values()){
    const factor = v.section === 'MICROXS' ? 0.5 : 1;
    for (const [wk, qty] of v.weeklyTotals.entries()){
      STATE.marketWeekly.set(wk, (STATE.marketWeekly.get(wk)||0) + qty*factor);
    }
  }
  for (const p of STATE.production){
    ...
    // Décalage ETH manuel : si l'utilisateur a réglé +1/-1 (ou plus) pour
    // cette variété, on applique le même décalage à TOUTES ses dates de
    // production, sans toucher aux autres variétés.
    const offset = STATE.ethOffsets[key] || 0;
    const shiftedDate = p.date + offset;
    v.productionParDate.set(shiftedDate, (v.productionParDate.get(shiftedDate)||0) + qty);
  }
}
...
const CONFIDENCE_WINDOW_WEEKS = 4; // fenêtre autour de la semaine "maintenant" l'an dernier, pour compter les clients distincts
...
function computeWeeklyForecast(v, weekStart, refWeekStart){
  ...
  // Momentum : an dernier, "semaine visée" vs "maintenant".
  const lastYearNowWeek = refWeekStart - 364;
  const lastYearTargetWeek = weekStart - 364;
  const varietyNowVol = v.weeklyTotals.get(lastYearNowWeek) || 0;
  const varietyTargetVol = v.weeklyTotals.get(lastYearTargetWeek) || 0;
  const marketNowVol = STATE.marketWeekly.get(lastYearNowWeek) || 0;
  const marketTargetVol = STATE.marketWeekly.get(lastYearTargetWeek) || 0;

  const marketMomentum = marketNowVol > 0 ? marketTargetVol / marketNowVol : 1;
  const varietyMomentum = varietyNowVol > 0 ? varietyTargetVol / varietyNowVol : marketMomentum;

  // Confiance : nb de clients distincts sur une fenêtre autour de la semaine
  // "maintenant" l'an dernier.
  const clientSet = new Set();
  for (let w=-CONFIDENCE_WINDOW_WEEKS; w<=CONFIDENCE_WINDOW_WEEKS; w++){
    const s = v.weeklyClients.get(lastYearNowWeek + 7*w);
    if (s) for (const c of s) clientSet.add(c);
  }
  const confidence = clientSet.size;
...
function projectVarietyFull(key){
  ...
  for (let i=0; i<TARGET_CALENDAR_DAYS; i++){
    ...
    days.push({ date: d, stock: running, production: prod, vente, venteReelle, isMonday: dow===1, isWeekend: dow===0 || dow===6, isOverride: hasOverride });
  }
...
async function savePersistedState(){
  try{
    ...
    logImport('Stock de départ, décalages ETH et comptages forcés sauvegardés.');
    return true;
  }catch(e){
    console.error(e);
    alert("Erreur de sauvegarde (" + e.message + "). Vérifie ta connexion et tes droits d'accès au Sheet « Wesh Grow - App State ».");
    return false;
  }
}
