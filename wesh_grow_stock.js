...
function rebuildVarieties(){
  ...
  for (const s of STATE.sales){
    ...
    // Exclusion stricte des ventes Marseille, même si le code partage le même
    // "Code Maj" que la variété Paris correspondante.
    if (norm.secteurMaj && /marseille/i.test(norm.secteurMaj)) continue;
    const v = STATE.varieties.get(key);
    let qty = s.qty;
    if (/colis/i.test(s.codeRaw)) qty = qty * colisMultiplier(v.section);
