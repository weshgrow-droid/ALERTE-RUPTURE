...
const APP_VERSION = 'v2026-09-10-14h50-debugvalues';
...
function buildForecastModel(v, refSerial){
  ...
  const demandMap = v.demandParDate || v.ventesParDate;
  for (let d=0; d<7; d++){
    const date = refWeekStart + d;
    if (date < refSerial && demandMap.has(date)){
      realizedActual += demandMap.get(date);
  ...
  if (realizedExpected > 0){
    ...
    // ici la correction vient d'un chiffre précis et volontaire (ventes
    // réelles + demande manquée déclarée), pas d'une estimation floue à
    // sécuriser — juste un garde-fou contre un dérapage extrême.
    currentWeekRatio = Math.max(CURRENT_WEEK_RATIO_MIN, Math.min(CURRENT_WEEK_RATIO_MAX, currentWeekRatio));
  }
  const adjustedBase = tendance * currentWeekRatio;
