...
const APP_VERSION = 'v2026-09-03-03h05-synthesis';
...
function renderRuptureSynthesis(){
  ...
  // Pour chaque semaine (en cours, +1, +2), on additionne le déficit (Manque)
  // de chaque variété sur les 7 jours de cette semaine, puis on trie du plus
  // gros déficit au plus petit.
  ...
  const weeksData = [0, 1, 2].map(w => {
    ...
    for (const key of keys){
      ...
      for (const day of proj.days){
        ...
        if (day.isShortfall) deficit += day.rawStock; // négatif
      }
      if (deficit < 0) rows.push({ nom: v.nom, deficit });
    }
    rows.sort((a,b) => a.deficit - b.deficit); // plus gros déficit (le plus négatif) en premier
    return { label: weekLabels[w], start: wStart, end: wEnd, rows };
  });

  const cols = weeksData.map(wd => {
    const items = wd.rows.length === 0
      ? `<div class="small" style="color:var(--muted);padding:6px 0;">Aucune rupture prévue</div>`
      : wd.rows.map(r => `<div style="display:flex;justify-content:space-between;gap:10px;padding:5px 0;border-bottom:1px solid var(--border);font-size:12px;">
          <span>${r.nom}</span>
          <b style="color:var(--danger);white-space:nowrap;">${Math.round(r.deficit)}</b>
        </div>`).join('');
