...
const APP_VERSION = 'v2026-09-03-02h20-prevweek';
...
function projectVarietyFull(key){
  ...
  const days = [];
  // La semaine précédente (masquée par défaut) est calculée à rebours depuis
  // le stock de départ saisi pour la semaine en cours — c'est donc une
  // valeur approximative, juste pour se souvenir de la tendance, pas un vrai
  // comptage historique. On repart bien du stock exact au lundi de la
  // semaine en cours, sans que ce calcul rétroactif ne le perturbe.
  let running = startStock;
  for (let i=0; i<TARGET_CALENDAR_DAYS; i++){
    const d = displayStart + i;
    if (d === weekStart) running = startStock;
...
function renderKPIs(){
  ...
  for (const k of keys){
    ...
    if (proj.days.some(d => d.isShortfall)) ruptureCount++;
  }
  const kpis = [
