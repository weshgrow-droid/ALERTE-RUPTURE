...
const APP_VERSION = 'v2026-09-02-22h50-ethoffset';
...
function projectVarietyFull(key){
  ...
  for (let i=0; i<TARGET_CALENDAR_DAYS; i++){
    ...
    days.push({ date: d, stock: running, production: prod, vente, venteReelle, isMonday: dow===1, isWeekend: dow===0 || dow===6 });
  }
...
async function loadPersistedState(){
  try{
    ...
    // sans le niveau stockManuel/ethOffsets.
    if (parsed && (parsed.stockManuel || parsed.ethOffsets)){
...
async function savePersistedState(){
  try{
    await sheetsUpdateValues(APP_STATE_CONFIG.spreadsheetId, `'${APP_STATE_CONFIG.sheetTitle}'!A2:B2`,
      [['stockManuel', JSON.stringify({ stockManuel: STATE.stockManuel, ethOffsets: STATE.ethOffsets })]]);
    logImport('Stock de départ et décalages ETH sauvegardés.');
    return true;
  }catch(e){
    console.error(e);
...
function renderStockEditor(){
  ...
  for (const sec of allSections()){
    ...
    for (const key of keys){
      ...
      const offset = STATE.ethOffsets[key] || 0;
      inner += `<div style="display:flex;flex-direction:column;gap:2px;font-size:11px;color:var(--muted);min-width:150px;">
        <label>${v.nom}
        ...
        <div style="display:flex;align-items:center;gap:3px;margin-top:2px;">
          ...
          <button type="button" class="eth-btn" data-key="${key}" data-dir="-1" title="Décaler -1 jour" style="padding:2px 6px;line-height:1;">▼</button>
          <input type="number" class="eth-offset-input${offset!==0?' dirty':''}" data-key="${key}" value="${offset}" style="width:44px;text-align:center;">
          <button type="button" class="eth-btn" data-key="${key}" data-dir="1" title="Décaler +1 jour" style="padding:2px 6px;line-height:1;">▲</button>
        </div>
      </div>`;
    }
  ...
  return `
    <div class="panel">
      <details>
        <summary>Stock de départ &amp; réglage ETH — modifier</summary>
        <p class="hint">Stock : pré-rempli depuis le dernier inventaire connu, à ajuster à la main. ETH : décale TOUTES les dates de production de cette variété de N jours (utile si la date réelle diffère de celle du Sheet). Clique « Enregistrer le stock de départ » en haut pour sauvegarder les deux.</p>
        ${inner}
      </details>
    </div>`;
}
